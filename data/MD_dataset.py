...
class MDDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super().__init__()
        print(f"DEBUG: opt keys = {opt.keys()}")
        print(f"DEBUG: dataroot = {opt.get('dataroot', 'NOT SET')}")
        print(f"DEBUG: phase = {opt.get('phase', 'NOT SET')}")
        print(f"DEBUG: distortion = {opt.get('distortion', 'NOT SET')}")
        print(f"DEBUG: data_type = {opt.get('data_type', 'NOT SET')}")
        self.opt = opt
        ...
        for deg_type in opt["distortion"]:
            if opt["phase"] == "train":
                GT_paths = util.get_image_paths(
                    opt["data_type"], os.path.join(opt["dataroot"], deg_type, 'train', 'GT')
                )
                LR_paths = util.get_image_paths(
                    opt["data_type"], os.path.join(opt["dataroot"], deg_type, 'train', 'LQ')
                )
            else:  # validation
                GT_paths = util.get_image_paths(
                    opt["data_type"], os.path.join(opt["dataroot"], deg_type, 'val', 'GT')
                )
                LR_paths = util.get_image_paths(
                    opt["data_type"], os.path.join(opt["dataroot"], deg_type, 'val', 'LQ')
                )
    ...
    def __getitem__(self, index):
        # Find which degradation type and actual index without repetition
        cumsum = 0
        deg_type = None
        actual_index = 0
        
        for type_id, dt in enumerate(self.deg_types):
            if index < cumsum + self.data_lens[type_id]:
                deg_type = dt
                actual_index = index - cumsum
                break
            cumsum += self.data_lens[type_id]
        
        # Get GT image
        GT_path = self.distortion[deg_type][0][actual_index]
        img_GT = util.read_img(None, GT_path, None)

        # Get LQ image
        LQ_path = self.distortion[deg_type][1][actual_index]
